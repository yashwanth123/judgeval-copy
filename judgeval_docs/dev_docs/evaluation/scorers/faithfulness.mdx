...
TODO plug our evaluation foundation models here
...
`Faithfulness` scores are calculated by first extracting all statements in `actual_output` and then classifying which ones are contradicted by the `retrieval_context`. 
