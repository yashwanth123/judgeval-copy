...
The Model Context Protocol (MCP) is an open standard designed to unify how applications deliver context and tools to large language models (LLMs). 
MCP integrates easily with Judgment, as LLMs can provide more valuable insights to help improve workflows and agents with our MCP server. 
For more information about MCP, refer to the official [documentation](https://modelcontextprotocol.io/introduction).

## Why use our MCP server?

The MCP server allows an LLM you are working with to easily gain key insights about workflows and agents, including information about traces and evaluations 
allow for a much better understanding of what is working well and what needs improvement. In addition, one other benefit, if integrating with an AI powered 
code editor, is that the LLM will now have a much better idea of how to approach code optimization. For example, imagine if you have a workflow with 1000 traces and 
something is not working. You can ask your AI powered IDE for analysis on what is going wrong with the traces, and then ask the IDE to optimize your code based on the 
insights! This greatly reduces the amount of work you have to do while running experiments or monitoring your workflow with Judgment.

## How do I use the MCP server?

Right now, MCP is a relatively new protocol, and many AI agents have still not added integration with MCP server. Some common ways to integrate our MCP server are 
with Cursor, Windsurf, and Claude Desktop. To integrate our MCP server with an AI agent, use the following json file as the MCP config (using Docker):
```
{
    "mcpServers": {
        "mcp-server": {
...
```
Note: With the current implementation of our MCP server, you will need to also pass in an API key for an LLM. We recommend using Gemini if 
you have an API key for it, but we also support OpenAI or Anthropic. If using Anthropic, pass in ANTHROPIC_API_KEY for the -e flag. If using 
OpenAI, pass in OPENAI_API_KEY for the -e flag.

Ensure that the MCP server is enabled (may take a couple seconds for the Docker pull). 

### Rule File

If your AI powered IDE supports it, we strongly recommend adding a rule file which helps the AI agent better understand the tools and 
how to optimize your code:

<Accordion title="Judgment Rule File">
    ````
    ---
...
`get_experiment_analysis` : Get experiment analysis for a single experiment, given the project and experiment names.

`get_trace_analysis` : Get trace analysis for a single trace, given the trace id.

`get_experiment_analysis_project` : Get experiment analysis for a full project, given the project name. Optionally, can also specify 
the value k, where the analysis will be for the last k experiments. k is 10 by default.

`get_trace_analysis_project` : Get trace analysis for a full project, given the project name. Optionally, can also specify 
the value k, where the analysis will be for the last k traces. k is 10 by default.

`run_evaluation` : Run an evaluation directly through an MCP tool! Specify the project name and as many fields as you would like to 
when it comes to the other EvaluationRun object fields. For example, you can specify all of the information for each of the examples and 
scorers and which model to use, or you can simply just ask the AI agent to create the examples based on a specific prompt. Think of this tool 
as a way to very quickly experiment any idea you may have.