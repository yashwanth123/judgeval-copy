...
- `use_judgment` (Python) / `useJudgment` (Typescript): Whether to use Judgment's infrastructure to execute the evaluation. Defaults to `true`.
...
- `asyncExecution` (Typescript only): Whether to run the evaluation asynchronously. Defaults to `false`.
