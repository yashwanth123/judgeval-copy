...
If none of `judgeval`'s built-in scorers fit your evaluation criteria, you can easily build your own custom metric to be run through a `CustomScorer`. 
`CustomScorer`s are automatically integrated within `judgeval`'s infrastructure, so you can:
...
Be creative with your custom scorers! You can measure anything you want using a `CustomScorer`, 
...
1. Inherit from the `CustomScorer` class
...
`CustomScorer`s have some required attributes that must be determined in the `__init__()` method. 
...
That's it! Congratulations, you've made your first custom scorer! Now that your scorer is implemented, you can run it on your own datasets 
just like any other `judgeval` scorer. Your scorer is fully integrated with `judgeval`'s infrastructure so you can view it on the Judgment platform too.
