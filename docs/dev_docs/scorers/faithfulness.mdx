...
The faithfulness metric measures the quality of your RAG pipeline's generator by evaluating whether the `actual_output` factually aligns with the contents of your `retrieval_context`. `judgeval`'s faithfulness metric is a self-explaining LLM-Eval, meaning it outputs a reason for its metric score.

:::info
Although similar to the `HallucinationMetric`, the faithfulness metric in `judgeval` is more concerned with contradictions between the `actual_output` and `retrieval_context` in RAG pipelines, rather than hallucination in the actual LLM itself.
:::
...
```python
from judgeval.judgeval import evaluate
from judgeval.judgeval.metrics import FaithfulnessMetric
from judgeval.judgeval.test_case import LLMTestCase
...
- [Optional] `model`: a string specifying which of OpenAI's GPT models to use, **OR** [any custom LLM model](metrics-introduction#using-a-custom-llm) of type `judgevalBaseLLM`. Defaulted to 'gpt-4o'.
