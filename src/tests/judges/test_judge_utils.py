...
def test_create_judge_invalid_type():
    ...
    with pytest.raises(InvalidJudgeModelError, match="Model must be a string, list of strings, or a judgeval judge object"):
        create_judge(model=42)

def test_create_judge_invalid_model_name():
    # Test invalid model name string
    with pytest.raises(InvalidJudgeModelError, match="Invalid judge model chosen: invalid-model"):
...
def test_create_judge_litellm_model():
    ...
    judge, is_native = create_judge(model="gpt-4")  # Assuming gpt-4 is in LITELLM_SUPPORTED_MODELS
    assert isinstance(judge, LiteLLMJudge)
