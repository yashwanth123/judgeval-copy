...
def run_rules(local_results: List[ScoringResult], rules: List[Rule], judgment_api_key: str, organization_id: str) -> None:
    """
    Evaluates rules against local scoring results and logs any triggered alerts.
    
    Args:
        local_results (List[ScoringResult]): The local scoring results to evaluate against
        rules (List[Rule]): The rules to evaluate
        judgment_api_key (str): The API key for judgment service
        organization_id (str): The organization ID
        
    Returns:
        None
    """
    info(f"Evaluating {len(rules)} rules against local scoring results")
    try:
        # Convert list of rules to dictionary for RulesEngine
        rules_dict = {f"rule_{i}": rule for i, rule in enumerate(rules)}
        ...
        for example_id, rule_results in all_rule_results.items():
            for rule_id, alert_result in rule_results.items():
                ...
                if "judgment_api_key" not in alert_result.metadata:
                    alert_result.metadata["judgment_api_key"] = judgment_api_key
                
                # Check if notification config is set for this rule
                for rule in rules:
                    if rule.rule_id == alert_result.rule_id and rule.notification and alert_result.status == AlertStatus.TRIGGERED:
                        # Only add notification config for triggered alerts
                        notification_config = rule.notification
                        
                        # Ensure notification config uses the new structure
                        if isinstance(notification_config, dict):
                            # Update channels to communication_methods if present
                            if 'channels' in notification_config and 'communication_methods' not in notification_config:
                                notification_config['communication_methods'] = notification_config.pop('channels')
                                
                            # Remove severity if present (deprecated)
                            if 'severity' in notification_config:
                                notification_config.pop('severity')
                                
                            # Ensure delay and fixed_time fields are preserved
                            for field in ['delay', 'fixed_time', 'delay_minutes', 'delay_hours', 'delay_days']:
                                if field in rule.notification and field not in notification_config:
                                    notification_config[field] = rule.notification[field]
                            
                        alert_result.notification = notification_config
                        debug(f"Added notification config to alert for rule '{alert_result.rule_name}'")
                ...
                if alert_result.metadata:
                    ...
                    alert = {
                        ...
                        "metadata": alert_result.metadata.copy()  # Create a copy of metadata
                    }
                    
                    # Remove judgment_api_key from alert metadata since we'll use the Authorization header
                    if "judgment_api_key" in alert["metadata"]:
                        alert["metadata"].pop("judgment_api_key")
                    
                    # Add notification config if present
                    if alert_result.notification:
