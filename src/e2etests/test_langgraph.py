...
async def async_call_llm_node(state: SimpleState) -> SimpleState:
    ...
    print(f"--- Node: async_call_llm_node ---")
...
def fetch_and_validate_trace(trace_id: str, expected_project: str):
    ...
    if not API_KEY or not ORG_ID:
        pytest.skip("JUDGMENT_API_KEY or JUDGMENT_ORG_ID not set, skipping trace fetch.")
    ...
    try:
        ...
        assert "trace_spans" in trace_data, f"Fetched trace {trace_id} should contain 'trace_spans' key."
        trace_spans = trace_data.get("trace_spans")
        assert trace_spans is not None, f"Trace {trace_id} field 'trace_spans' should not be None."
        ...
        if trace_spans is None: # Should not happen due to assert above, but defensive
            warnings.warn(f"Trace {trace_id} fetched successfully, but 'trace_spans' was None. Skipping specific span assertions.")
            span_functions = []
        else:
...
def test_sync_graph_execution():
    ...
    try:
        ...
        trace_client_sync = getattr(handler_sync, '_trace_client', None) # Use getattr for safety
        assert trace_client_sync is not None, "Sync handler should have an active trace client after invoke."
        ...
        if hasattr(handler_sync, 'executed_nodes'):
            ...
            warnings.warn("executed_nodes attribute not found on sync handler. Skipping node execution assertion.")
    ...
    if trace_id_sync:
        ...
        assert fetched_trace.get("trace_spans") is not None, f"Trace {trace_id_sync} should have trace_spans."
        assert any("call_llm" in span.get("function","") for span in fetched_trace["trace_spans"]), f"Trace {trace_id_sync} should contain 'call_llm' span."
        assert any("OPENAI_API_CALL" in span.get("function","") for span in fetched_trace["trace_spans"]), f"Trace {trace_id_sync} should contain 'OPENAI_API_CALL' span."
...
async def test_async_graph_execution():
    ...
    try:
        ...
        result = await graph_async.ainvoke(initial_state, config=config) # USE graph_async

        # --- Assertions ---
        # Check node execution via handler AFTER invoke
        if hasattr(handler_async, 'executed_nodes'):
            assert "async_call_llm" in handler_async.executed_nodes # Check for async node
            assert "finish" in handler_async.executed_nodes
        else:
            warnings.warn("executed_nodes attribute not found on async handler. Skipping node execution assertion.")
        ...
        assert result.get("result") == "Async LLM Called" # Expect result from async node

        # Get trace_id AFTER execution
        trace_client_async = getattr(handler_async, '_trace_client', None) # Use getattr for safety
        assert trace_client_async is not None, "Async handler should have an active trace client after ainvoke."
    ...
    assert trace_id_async is not None, "trace_id_async was not set during the async test run."
    if trace_id_async:
        # Add a small delay before fetching
